            ...
            if(endstop_z_hit == true)
            {
		        ...
		        if (i > 0 ) //Second probe has happened so compare results
		        {
		            ...
		            { //keep going until readings match to avoid sticky bed
		              ...
		              SERIAL_ECHO(ProbeDepth[i]); SERIAL_ECHO(" - "); SERIAL_ECHO(ProbeDepth[i - 1]);
		              SERIAL_ECHO(" = "); SERIAL_ECHOLN(abs(ProbeDepth[i] - ProbeDepth[i - 1]));
//		              meas = ProbeDepth[i];
...
void probe_3points()
{
    ...
    Point2 = Probe_Bed(15,Y_MAX_LENGTH - 15,PROBE_N) ;
    Point3 = Probe_Bed(X_MAX_LENGTH - 15,Y_MAX_LENGTH/2,PROBE_N);
